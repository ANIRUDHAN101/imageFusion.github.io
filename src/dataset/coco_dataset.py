...
class DataAugment():
    ...
    def __iter__(self):
        for i, data in enumerate(self.dataloader):
            ...
            # random blur
            if self.random_blur:
                input_img_1, input_img_2, input_mask = blur_with_boundary(data[0].to(self.device), data[1].to(self.device))
            else:
                input_img_1, input_img_2, input_mask = blur_with_boundary(data[0].to(self.device), data[1].to(self.device), filter_size=self.filter_sizes[i])
            # random erasing
            ...
            yield data[0], input_img_1, input_img_2, input_mask
